...
class District():
    ...
    def get_crop_details(self,crops,i):
        ...
        for crop_id,j in zip(crops.keys(),range(len(crops.keys()))):
            temp.append([i,j,crops[crop_id].cost_dict[self.id],crops[crop_id].yield_dict[self.id],crops[crop_id].price_dict[self.id],crops[crop_id].demand_max_dict[self.id],crops[crop_id].demand_min_dict[self.id],crops[crop_id].yield_dict_n[self.id],crops[crop_id].price_dict_n[self.id]])
        return temp
