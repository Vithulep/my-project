...
def read_files(example_path):
    ...
    warehouses_numpy_new= warehouses_numpy
    ...
    # print(dist_crop_numpy)
       #-------------------new_allocation---------------------------
    new_allocation_wh = readfile.read_allocation(example_path,'new_allocation.csv',crops)
    new_allocation_wh_numpy = []               #rows -> districts, columns -> crops
    for i in new_allocation_wh.allocation.keys():
        temp = []
        for j in crops.keys():
            temp.append(new_allocation_wh.allocation[i][j])  
        new_allocation_wh_numpy.append(temp)
    new_allocation_wh_numpy = np.array(new_allocation_wh_numpy)
    print(new_allocation_wh_numpy.shape)
    print(new_allocation_wh_numpy)

    #-------------------new_allocation---------------------------
    new_allocation = readfile.read_allocation(example_path,'new_allocation_wh.csv',crops)
    ...
    return district_numpy,warehouses_numpy,crops_numpy,dist_crop_numpy,standard_allocation_numpy,new_allocation_numpy,districts,crops,new_allocation,dist_ware_cap_numpy,new_allocation_wh_numpy
...
def print_to_ware_file(districts,crops,alloc,new_allocation_wh):
    for district,i in zip(districts.values(),range(len(districts.values()))):
                for crps,j in zip(list(crops.keys()),range(len(crops.keys()))):
                    new_allocation_wh.allocation[district.id][crps]=alloc[i,j]
    # print(f"Allocation: {new_allocation.allocation}")
    allocation_to_csv(new_allocation_wh)

def plot_all_metrics(alloc1,current_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,stored_warehouse,dist_war_cap,price_n,y):
    ...
    for c in range(11):
        ...
        p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,stored_warehouse, dist_war_cap,price_n,y, allocation,optimize=0)
...
def plot_max_min(no_dist,no_crop,cost,crop_yield,price,tc,distances,max_alloc,standard_allocation_numpy,ld,ud,stored_warehouse,warehouse_numpy,price_n,y):
    for i in range(5):
        ld_factor = (0.9-0.1*i)
        # ud_factor = (1.1+0.1*i)
        ud_factor = 1.2
        ...
        p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_temp,ud_temp,tc,distances,max_alloc,stored_warehouse,warehouse_numpy,price_n,y,allocation=0,optimize=1)
        profit_vector = profit_complaince(alloc,standard_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld_temp,ud_temp,tc,distances,max_alloc,stored_warehouse,warehouse_numpy,price_n,y)
...
def profit_complaince(alloc1,standard_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,stored_warehouse,warehouse_numpy,price_n,y):
    recommended_allocation = alloc1
    profit_vector = []
    for c in range(11):
        print(f'---------------------{c/10}-----------------------')
        allocation = (1-c/10)*standard_allocation_numpy + (c/10)*recommended_allocation
        p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,stored_warehouse,warehouse_numpy,price_n,y,allocation,optimize=0)
        profit_vector.append(Profit/1e9)
    return profit_vector

def gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,LD,UD,TC,distances,max_alloc,stored_warehouse,dist_war_cap,price_n,y, allocation,optimize):
    #----------------------Gurobi code----------------------
    ...
    whalloacation = model.addVars(no_dist, no_crop,vtype=GRB.CONTINUOUS,lb = 0, name = 'whalloacation') # For Warehouse Allocation
    wx = model.addVars(no_dist,no_crop*no_dist,vtype=GRB.CONTINUOUS,lb = 0, name = 'wx') # for WArehouse crop allocation  
    ...
    model.addConstrs((allocation[i,j]*crop_yield[i,j]-sum(x[i,j*no_dist+k] for k in range(no_dist))+sum(x[k,j*no_dist+i]\
                    for k in range(no_dist)))>=LD[i,j] + y[i,j] for i in range(no_dist) for j in range(no_crop))
    
    model.addConstrs((allocation[i,j]*crop_yield[i,j]-sum(x[i,j*no_dist+k] for k in range(no_dist))+sum(x[k,j*no_dist+i]\
                    for k in range(no_dist)))<=UD[i,j] for i in range(no_dist) for j in range(no_crop))
    
    model.addConstrs((sum(whalloacation[i,j] for j in range(no_crop))<=dist_war_cap[i]) for i in range(no_dist))
    ...
    Revenue_wh = quicksum(whalloacation[i,j]*0.95*price_n[i,j] for i in range(no_dist) for j in range(no_crop)) 
    Transport_cost_wh = quicksum((TC[j]*distances[i,k]*wx[i,j*no_dist+k]) for k in range(no_dist) for i in range(no_dist) for j in range(no_crop))
    ...
    model.setObjective(obj_fn_wh,GRB.MAXIMIZE)
    model.optimize()
    ...
    if model.status == GRB.Status.OPTIMAL:
        ...
        if optimize == 1:
            ...
            warealloc = values[no_crop*no_dist+no_dist*no_dist*no_crop:no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_crop].reshape((no_dist,no_crop)) 
            # warehouse transport
            ware_transport = values[no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_crop:no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_crop+no_dist*no_crop*no_dist].reshape((no_dist, no_crop*no_dist)) 
            # print(f'Allocation: \n {alloc}')
            # print(f'Allocation: \n {alloc}')
            ...
            # warehouse allocation 
            warealloc = values[no_dist*no_dist*no_crop:no_dist*no_dist*no_crop+no_dist*no_crop].reshape((no_dist,no_crop)) 
            # warehouse transport
            ware_transport = values[no_dist*no_dist*no_crop+no_dist*no_crop:no_dist*no_dist*no_crop+no_dist*no_crop+no_dist*no_crop*no_dist].reshape((no_dist, no_crop*no_dist)) 
            # print(f'Allocation: \n {alloc}')
...
def main():
    ...
    district_numpy,warehouses_numpy, crops_numpy,dist_crop_numpy,standard_allocation_numpy,new_allocation_numpy,districts,crops,new_allocation,dist_ware_cap_numpy,warehouse_numpy_new = read_files(example_path)
    ...
    ware_updated= dist_ware_cap_numpy*1.1
    ...
    p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,stored_warehouse,dist_ware_cap_numpy,price_n,ld1,allocation=0,optimize=1)
    print(Profit)
    ...
    # print_to_ware_file(districts,crops,whalloc,warehouse_numpy_new)
    

    #-----------------------------------Plotting revenue,TC,COP,Profit----------------------------------------
    plot_all_metrics(alloc,new_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,stored_warehouse,dist_ware_cap_numpy,price_n,ld1)
    ...
    # plot_max_min(no_dist,no_crop,cost,crop_yield,price,tc,distances,max_alloc,standard_allocation_numpy,ld_updated,ud_updated,stored_warehouse,dist_ware_cap_numpy,price_n,ld1)
