...
def plot_all_metrics(alloc1,current_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,alloc_1):
    ...
    for c in range(11):
        ...
        allocation_1 = (1-c/10)*current_allocation_numpy + (c/10)*recommended_allocation_1
        p,r,t,Profit,alloc,whalloc,alloc_2,qty = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,allocation,allocation_1,optimize=0)
...
def gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,LD,UD,TC,distances,max_alloc, no_warehouses, UBWarehouse, warehouse_storage_cost, distanceswh,crop_yield_n,price_n,wh_id,x_st,allocation,allocation_1,optimize):
    ...
    if optimize == 1: 
        ...
        model.addConstrs((sum(allocation_1[i,j] for j in range(no_crop))<=max_alloc[i]) for i in range(no_dist))
    ...
    model.addConstrs((sum(whalloacation_1[i,j] for j in range(no_crop))<=UBWarehouse[i]) for i in range(no_warehouses))
    ...
    rnw_war= Revenue_with_whs+Revenue_wh_2 + Revenue_wh_1+Revenue_with_whs_1+Revenue_wh_2 # 2 year revenue 
    ...
    if model.status == GRB.Status.OPTIMAL:
        ...
        if optimize == 1:
            ...
            # print(f'Allocation: \n {alloc}')
            cond_1 = values[no_dist*no_crop+no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses:no_dist*no_crop+no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+1].reshape((1)) 
            cond_2 = values[no_dist*no_crop+no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+1:no_dist*no_crop+no_crop*no_dist+no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+2].reshape((1)) 
            ...
            cond_1 = values[no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses:no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+1].reshape((1)) 
            cond_2 = values[no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+1:no_dist*no_dist*no_crop+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses+no_dist*no_crop*no_warehouses+2].reshape((1)) 
       ...
       #revenue 2019-20
        r_1 = sum((alloc_1[i,j]*crop_yield[i,j]-sum(transport_qty_1[i,j*no_dist+k] for k in range(no_dist))+sum(transport_qty_1[k,j*no_dist+i] for k in range(no_dist))-sum(ware_transport_1[i,j*no_warehouses+k] for k in range(no_warehouses)))*price_n[i,j] for i in range(no_dist) for j in range(no_crop))
        ...
        r_wh_1 = sum(warealloc_1[i,j]*0.95*price[wh_id[i]-1,j] for i in range(no_warehouses) for j in range(no_crop))
       ...
       #transportation 2018-19      
        ...
        print(Profit_1/1e9)
        print(r_1+r_wh_1/1e9)
        print(cond_1,cond_2)
    if Profit == 0.0:
        model.computeIIS()
    # return p, r+r_wh , t, Profit,alloc,warealloc,alloc_1    
    return p+p_1,r+r_1+r_wh+r_wh_1,t+t_1+t1+t1_1,Profit+Profit_1,alloc,warealloc,alloc_1,transport_qty
...
#    print(distances)
    ...
    ud = dist_crop_numpy[:,:,5]*(1.2/1.1)*1.3  #Maximum dwmand of district in tonnes (110% of district-wise production)
    ...
    # print(crop_yield -crop_yield_n)
    # return 0
    np.set_printoptions(suppress=True)

    #-------------------------------------------Basic optimizer code-------------------------------------------------
    p,r,t,Profit,alloc,whalloc,alloc_1,transport_qty= gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,allocation=0,allocation_1=0,optimize=1)
    print(Profit)
    print(r)

    final_store_ud = np.zeros((30,12)) 

    for i in range(no_dist):
        for j in range(no_crop):
            a1= 0
            a2= 0 
            for k in range(no_dist): 
                a1+= transport_qty[i,j*no_dist+k] 
                a2+= transport_qty[k,j*no_dist+i]
                # final_store_ud[i,j]-= transport_qty[i,j*no_dist+k] 
                # final_store_ud[i,j]+= transport_qty[k,j*no_dist+i]
            final_store_ud [i,j]= alloc[i,j]*crop_yield[i,j] - a1 + a2 
    
    print(final_store_ud -ud_updated)
    print(final_store_ud - ud_updated<=0)
    # return 0
    # print(alloc)
    # print(alloc_1)
    # print(warehouse_storage_cost)
    ...
    alloc_1yr = 0.8*alloc + 0.2*new_allocation_numpy
    alloc_2yr = 0.8*alloc_1 + 0.2*new_allocation_numpy 
    p_x,r_x,t_x,Profit_x,alloc_x,whalloc_x,alloc_1_x,qty = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,alloc_1yr,alloc_2yr,optimize=0)
    ...
    for i in range(11): 
        ...
        alloc_2yr = A[i]*alloc_1 + (1-A[i])*alloc_1yr
        p_x,r_x,t_x,Profit_x,alloc_x,whalloc_x,alloc_1_x,qty = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,alloc_1yr,alloc_2yr,optimize=0)
