...
def plot_all_metrics(alloc1,current_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x):
    ...
    for c in range(11):
        ...
        p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld,ud,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,allocation,optimize=0)
...
def gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,LD,UD,TC,distances,max_alloc, no_warehouses, UBWarehouse, warehouse_storage_cost, distanceswh,crop_yield_n,price_n,wh_id,x,allocation,optimize):
    #----------------------Gurobi code----------------------
    ...
    # Constraint on warehouse storage for each crop in each warehouse 
    # model.addConstrs((whalloacation[i,j] <= UB[i,j]) for j in range(no_crop) for i in range(no_warehouses))   

    # added storage constraint warehouse wise not warehouse*crop wiase for 2018-19 and 2019-20
    model.addConstrs((sum(whalloacation[i,j] for j in range(no_crop))<=UBWarehouse[i]) for i in range(no_warehouses))
   

    # model.addConstrs((sum(whalloacation[i,j] for i in range(n))>=0.1*UBWarehouse[i]) for i in range(no_warehouses))
    ...
    Revenue_wh = quicksum(x[i,j]*0.95*price[i,j] for i in range(no_dist) for j in range(no_crop)) 
    val=0.0

    #-------------- 2018-19 selled warehouse stored crops in 2017-18 
    for i in range(no_dist): 
        for j in range(no_crop):
            val=val+price[i,j]*x[i,j]*0.95
    ...
    +sum(x[k,j*no_dist+i] for k in range(no_dist)))*price[i,j] -condition*penalty for i in range(no_dist) for j in range(no_crop))
    
    #stock available in market revenew for 2019-20
    # print(Revenue_wh.type)

    Revenue_with_warehse=Revenue_x+Revenue_wh_1
    # rnw_war= Revenue_with_whs+Revenue_wh + Revenue_wh_1+Revenue_with_whs_1  # 2 year revenue 
    ...
    totat_TC = Transport_cost + Transport_cost_wh  # 2 year transportation cost 

    # objective function with warehoouse 
    print("..........transporation cost of warehouse....................")
    # print(Transport_cost)
    print("\n")
    # obj_fn_1 = Revenue_with_warehse - totat_TC -cop - warehouse_storage_cost
    ...
    if model.status == GRB.Status.OPTIMAL:
        ...
        if optimize == 1:
            ...
            warealloc = values[no_crop*no_dist+no_dist*no_dist*no_crop:no_crop*no_dist+no_dist*no_dist*no_crop+no_warehouses*no_crop].reshape((no_warehouses,no_crop)) 
            # warehouse transport
            ware_transport = values[no_crop*no_dist+no_dist*no_dist*no_crop+no_warehouses*no_crop:no_crop*no_dist+no_dist*no_dist*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses].reshape((no_dist, no_crop*no_warehouses)) 
            # print(f'Allocation: \n {alloc}')
            ...
            transport_qty = values[0:no_dist*no_dist*no_crop].reshape((no_dist,no_dist*no_crop)) # transportation quantity
            # warehouse allocation 
            warealloc = values[no_dist*no_dist*no_crop:no_dist*no_dist*no_crop+no_warehouses*no_crop].reshape((no_warehouses,no_crop)) 
            # warehouse transport
            ware_transport = values[no_dist*no_dist*no_crop+no_warehouses*no_crop:no_dist*no_dist*no_crop+no_warehouses*no_crop+no_dist*no_crop*no_warehouses].reshape((no_dist, no_crop*no_warehouses))

        p = sum(alloc[i,j]*cost[i,j] for i in range(no_dist) for j in range(no_crop))
        # rr_wh= sum(x[i,j]*0.95*price[i,j] for i in range(no_dist) for j in range(no_crop)) 
        r_wh = sum(warealloc[i,j]*0.95*price[wh_id[i]-1,j] for i in range(no_warehouses) for j in range(no_crop))
        ...
        print(t/1e9,t1/1e9)
        # pp = float(Profit)
        print(r_wh/1e9,r/1e9)
        print("......................................")
    if Profit == 0.0:
        model.computeIIS()
    return p,r+r_wh,t+t1,Profit,alloc,warealloc
...
#    print(distances)
    ...
    p,r,t,Profit,alloc,whalloc = gurobi_optimizer(no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield_n,price_n,wh_id,x,allocation=0,optimize=1)
    print(Profit)
    print(r)
    print(whalloc)
    # print(warehouse_capacity) 
    a =np.zeros(no_warehouse)
    for i in range(no_dist): 
        a[i]=sum(whalloc[i,j] for j in range(no_crop))

    print( a- warehouse_capacity <=0)  #<=warehouse_capacity[i]) for i in range(no_warehouse)
    return 0 
    # print(warehouse_storage_cost)
    # print(f"COP = {p}")
    # print(f'Revenue = {r}')
    ...
    plot_all_metrics(alloc,new_allocation_numpy,no_dist,no_crop,cost,crop_yield,price,ld_updated,ud_updated,tc,distances,max_alloc,no_warehouse,warehouse_capacity,warehouse_storage_cost,dist_warehouse,crop_yield,price_n,wh_id,x)
